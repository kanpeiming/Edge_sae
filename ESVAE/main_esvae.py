...
from svae_models.esvae import SobelEdgeExtractionModule
# 新增
...
def train(network, trainloader, opti, epoch):
    ...
    for batch_idx, (real_img, labels) in enumerate(trainloader):
        ...
        with torch.amp.autocast('cuda'):  # 使用 autocast 进行混合精度
            ...
            # loss = losses['loss']
            x_recon, r_q, r_p, sampled_z_q = network.module(spike_input, scheduled=glv.network_config['scheduled'])
            losses = network.module.loss_function_mmd(edge_img, x_recon, r_q, r_p)
            loss = losses['loss']
        ...
        print(
            f'Train[{epoch}/{max_epoch}] [{batch_idx}/{len(trainloader)}] Loss: {loss_meter.avg}, RECONS: {recons_meter.avg}, DISTANCE: {dist_meter.avg}, BOUNDARY: {boundary_meter.avg}')  # 新增, SSIM: {ssim_meter.avg:.7f}
    ...
    logging.info(
        f"Train [{epoch}] Loss: {loss_meter.avg} ReconsLoss: {recons_meter.avg} DISTANCE: {dist_meter.avg}, BOUNDARY: {boundary_meter.avg}")  # 删除DISTANCE: {dist_meter.avg}
...
def test(network, testloader, epoch):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (real_img, labels) in enumerate(testloader):
            ...
            print(
                f'Test[{epoch}/{max_epoch}] [{batch_idx}/{len(testloader)}] Loss: {loss_meter.avg}, RECONS: {recons_meter.avg}, DISTANCE: {dist_meter.avg}, BOUNDARY: {boundary_meter.avg}')  # 删除, DISTANCE: {dist_meter.avg}
    ...
    logging.info(
        f"Test [{epoch}] Loss: {loss_meter.avg} ReconsLoss: {recons_meter.avg} DISTANCE: {dist_meter.avg}, BOUNDARY: {boundary_meter.avg}")  # 删除 DISTANCE: {dist_meter.avg}
...
def sample(network, epoch, batch_size=64):  # 生成样本 128
    network = network.eval()
    with torch.no_grad():
        sampled_x, sampled_z_p = network.module.sample(batch_size)
        ...
        torchvision.utils.save_image((sampled_x + 1) / 2,
                                     f'{args.project_save_path}/checkpoint/{dataset_name}/{args.name}/imgs/sample/epoch{epoch}_sample.png')
...
if __name__ == '__main__':
    ...
    if network_config['model'] == 'ESVAE':
        net = esvae.ESVAE(device=init_device, distance_lambda=distance_lambda, mmd_type=mmd_type,
                          boundary_weight=boundary_weight)  # 设置 SSIM 权重
